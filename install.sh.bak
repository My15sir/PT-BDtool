...
bootstrap_fetch_repo() {
  ...
  if command -v git >/dev/null 2>&1; then
    bootstrap_msg "尝试 git clone 仓库..."
    if run_ext_boot 300 git clone --depth 1 --branch main https://github.com/My15sir/PT-BDtool.git "$repo_dir" >/dev/null 2>&1; then
      echo "$repo_dir"
      return 0
    fi
    bootstrap_msg "git clone 失败，回退归档下载。"
  fi
  ...
  if command -v curl >/dev/null 2>&1; then
    run_ext_boot 300 curl -fL --connect-timeout 10 --retry 3 --retry-delay 1 --retry-all-errors -o "$tarball" "$tar_url" >/dev/null 2>&1 || true
  fi
  if [[ ! -s "$tarball" ]] && command -v wget >/dev/null 2>&1; then
    run_ext_boot 300 wget -q -O "$tarball" "$tar_url" >/dev/null 2>&1 || true
  fi

  [[ -s "$tarball" ]] || return 1
  command -v tar >/dev/null 2>&1 || return 1
  run_ext_boot 300 tar -xzf "$tarball" -C "$tmp_root" >/dev/null 2>&1 || return 1
...
if [[ "${PTBD_BOOTSTRAP_DONE:-0}" != "1" && ! -f "$SCRIPT_DIR/lib/ui.sh" ]]; then
  ...
  cleanup_boot() {
    if [[ "$keep_tmp" == "1" ]]; then
      bootstrap_msg "KEEP_TMP=1，保留临时目录：$tmp_boot"
    else
  ...
  PTBD_BOOTSTRAP_DONE=1 bash "$repo_dir/install.sh" "${ORIG_ARGS[@]}"
  exit $?
fi
